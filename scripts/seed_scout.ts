...
const article = {
    slug: 'gw28-preview-ultimate-guide',
    gameweek: 28,
    title: 'FPL Gameweek 28 Preview: High-Stakes Differentials & Captaincy Strategy',
    summary: 'Master GW28 with our expert analysis on the best captain picks, differential gems, and the crucial transfer targets you need to climb the ranks.',
    content: `
...
The Gunners remain the most potent attacking force in the league. With Bukayo Saka and Martin Ødegaard pulling the strings, expect a high-scoring affair at the Emirates. Leicester's defensive lapses in transition will be exploited by Arsenal's high press.
...
Good luck, managers!
    `,
    captain_pick: 'Erling Haaland',
    differential_pick: 'Alex Iwobi',
    published: true,
    generated_at: new Date().toISOString()
};

async function seed() {
    console.log('Seeding Scout article for GW28...');
    ...
    if (error) {
        ...
        console.log('Successfully seeded GW28 Scout article!');
        console.log('View it at: http://localhost:3000/scout/' + article.slug);
    }
}
