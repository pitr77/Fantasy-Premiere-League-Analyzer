            ...
            {mobileView === 'STATS' ? (
               <div className="w-full overflow-x-hidden md:overflow-x-auto">
                  <table className="w-full text-left border-separate border-spacing-0 table-fixed">
                     <thead>
                        <tr className="bg-slate-900 text-slate-400 text-[10px] sm:text-xs uppercase tracking-wider">
                           <th className="p-3 sm:p-4 w-10 sm:w-12 md:w-16 text-center sticky left-0 z-20 bg-slate-900 border-b border-slate-700">Rank</th>
                           <th className="p-3 sm:p-4 w-20 sm:w-24 md:w-48 sticky left-10 sm:left-12 md:left-16 z-20 bg-slate-900 border-b border-slate-700 border-r border-slate-700">Team</th>
                           <th className="p-3 sm:p-4 text-center hidden md:table-cell border-b border-slate-700">Games</th>
                           {activeTab === 'ATTACK' ? (
                              <>
                                 <th className="p-3 sm:p-4 text-right border-b border-slate-700 w-[22%] sm:w-auto">Scored</th>
                                 <th className="p-3 sm:p-4 text-right border-b border-slate-700 text-white font-bold w-[28%] sm:w-auto">GS/G</th>
                                 <th className="p-3 sm:p-4 text-right border-b border-slate-700 hidden md:table-cell">FTS</th>
                              </>
                           ) : (
                              <>
                                 <th className="p-3 sm:p-4 text-right border-b border-slate-700 w-[22%] sm:w-auto">Conceded</th>
                                 <th className="p-3 sm:p-4 text-right border-b border-slate-700 text-white font-bold w-[28%] sm:w-auto">GC/G</th>
                                 <th className="p-3 sm:p-4 text-right border-b border-slate-700 hidden md:table-cell">CS</th>
                              </>
                           )}
                           <th className="p-3 sm:p-4 text-center hidden md:table-cell border-b border-slate-700 font-mono">GD</th>
                           <th className="p-3 sm:p-4 text-left pl-8 hidden md:table-cell border-b border-slate-700">Match History</th>
                        </tr>
                     </thead>
                     <tbody className="divide-y divide-slate-700/50">
                        {sortedData.map((team, idx) => {
                           ...
                           return (
                              <tr key={team.id} className="hover:bg-slate-700/30 transition-colors group text-[11px] sm:text-sm">
                                 <td className="p-3 sm:p-4 text-center text-slate-500 font-mono sticky left-0 z-10 bg-slate-800 transition-colors group-hover:bg-slate-700 border-b border-slate-700/50">#{idx + 1}</td>
                                 <td className="p-3 sm:p-4 font-bold text-white sticky left-10 sm:left-12 md:left-16 z-10 bg-slate-800 transition-colors group-hover:bg-slate-700 border-b border-slate-700/50 truncate border-r border-slate-700">
                                    <span className="md:hidden">{team.short_name}</span>
                                    <span className="hidden md:inline">{team.name}</span>
                                 </td>
                                 <td className="p-3 sm:p-4 text-center text-slate-300 hidden md:table-cell border-b border-slate-700/50">{team.played}</td>
                                 {activeTab === 'ATTACK' ? (
                                    <>
                                       <td className="p-3 sm:p-4 text-right text-slate-300 border-b border-slate-700/50">{team.scored}</td>
                                       <td className={`p-3 sm:p-4 text-right text-sm sm:text-lg font-mono border-b border-slate-700/50 ${statColor}`}>
                                          {perGame.toFixed(2)}
                                       </td>
                                       <td className="p-3 sm:p-4 text-right text-slate-400 hidden md:table-cell border-b border-slate-700/50">{team.failedToScore}</td>
                                    </>
                                 ) : (
                                    <>
                                       <td className="p-3 sm:p-4 text-right text-slate-300 border-b border-slate-700/50">{team.conceded}</td>
                                       <td className={`p-3 sm:p-4 text-right text-sm sm:text-lg font-mono border-b border-slate-700/50 ${statColor}`}>
                                          {perGame.toFixed(2)}
                                       </td>
                                       <td className="p-3 sm:p-4 text-right text-slate-400 hidden md:table-cell border-b border-slate-700/50">{team.cleanSheets}</td>
                                    </>
                                 )}
                                 <td className="p-3 sm:p-4 text-center font-mono text-slate-400 hidden md:table-cell border-b border-slate-700/50">
                                 ...
                                 <td className="p-3 sm:p-4 pl-8 hidden md:table-cell border-b border-slate-700/50">
